...
이미지 리사이징을 적용할 부분은 두가지이다. 첫번째는 프로젝트에서 고정적으로 사용되는 static asset 들이고, 두번째는 사용자가 업로드하는 상품의 이미지들이다.

static asset 에 해당하는 이미지들은 온라인 이미지 리사이징 서비스를 사용해 진행하고, 사용자가 업로드하는 상품의 이미지들은 이미지 최적화 과정의 자동화가 필요하기 때문에 따로 유틸리티 함수를 만들어서 원본 이미지와 리사이징된 이미지를 같이 업로드하는 방식을 선택했다. 대부분 프로젝트를 배포하는 플랫폼에는 이러한 이미지 최적화 과정에 대한 플러그인 기능이 있으나 유료가 많기 때문에 웹 프로젝트에서 최적화를 진행한다.
