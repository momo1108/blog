...
첫 번째 요소는 바로 Node.js 실행 경로[`process.execPath`](https://nodejs.org/docs/latest/api/process.html#processexecpath) 이고, 두 번째 요소는 실행될 JavaScript 파일의 경로이다.
...
이쪽 계열에 가장 인기가 많은 모듈을 찾아보니, 바로 [`Commander.js`](https://www.npmjs.com/package/commander) 라는 모듈이 있었다.
